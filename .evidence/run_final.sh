#!/usr/bin/env bash
# 最终实测：12 步演示路径 + 补证（扁平行证据面板 / 复制 / 导出 CSV）。
# 修正点：截图命名用 2 号位参数；断言前把多行文本压平（grep 是行匹配）；toast 即时读取。
set -u
REPO="/c/Users/hu/code/mst"
SD="C:/Users/hu/code/mst/.evidence"   # 必须用 Windows 风格路径：CLI 的 Node 不认 /c/... 形式
cd "$REPO" || exit 1
PASS=0; FAIL=0
flat(){ printf '%s' "$1" | tr '\n\r\t' '   ' | tr -s ' '; }
ck(){ local a; a=$(flat "$2");
...
shot(){ timeout 90 agent-browser screenshot "" "$SD/$1" >/dev/null 2>&1; echo "  截图: .evidence/$1"; }
...
echo "### 最终实测 $(date '+%F %T')  http://127.0.0.1:8000/"
...
"C:/Users/hu/.workbuddy/binaries/python/envs/mst/Scripts/python.exe" -c "
...
ck "STEP5a 台账行多列取值非空"    "$(ev "var t=document.querySelector('#evidence .ev-pane:last-child table');t?t.textContent:''")" '2098172078[\s\S]*100[\s\S]*200'
...
ck "STEP6 新 P0 为打款人字段冲突"  "$NEWMSG" '林老师[\s\S]*木木老师'
...
ck "STEP8 已确认/待确认金额并列"   "$(ab get text '#report-host')" '已确认金额[\s\S]*待确认金额'
ck "STEP8 抖加/垫付合计并列"      "$(ab get text '#report-host')" '抖加合计[\s\S]*垫付合计'
